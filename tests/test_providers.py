...
def test_xm_hourly_parser_supports_quarter_hours() -> None:
    ...
    payload = {"Items": [{"Date": "2026-08-07", "HourlyEntities": [{"Id": "Sistema", "Values": values}]}]}
    frame = XMProvider._parse_items([payload], container="HourlyEntities", frequency="hourly")
    assert len(frame) == 96
    assert frame["datetime"].iloc[-1] - frame["datetime"].iloc[0] == pd.Timedelta(hours=23, minutes=45)
    ...
    nested = [
        {
            ...
            "attributes": {
                "content": [
                    {"id": "hydro", "attributes": {"title": "Hidráulica", "values": [{"datetime": "2024-01-01", "value": 10}]}}
                ]
            },
        }
    ]
