...
def test_data_maintenance_explains_the_complete_update_flow() -> None:
    ...
    assert "Agregar meses y variables faltantes" in page
    assert "Construir la primera base" in page
    assert "Recalcular un periodo" in page
    assert "ZIP de avance" in page
    assert "Carpeta GitHub" in page
    assert "tres intentos" in page
    assert "Streamlit nunca modifica GitHub" in page
    assert "Se encontró la Demanda ya construida" not in page
    assert "administrador" not in page.lower()
...
"""
    ...
    assert not app.exception
    assert "Descargar ZIP completo para GitHub" in [
...
from came.data.automatic_maintenance import AutomaticBuildOutcome
...
class FakeBuilder:
    ...
    def build(self, *args, **kwargs):
        ...
        return AutomaticBuildOutcome(
            result=BuildResult(
                country="COL",
                data=pd.DataFrame([row]),
                status=pd.DataFrame({"Fuente": ["XM"], "Estado": ["Aprobado"]}),
            ),
            selected_options=sorted(kwargs.get("selected_options", set())),
            option_series={"demand": ["col_precio_prueba"]},
            completed_stages=["Demanda nacional"],
            run_complete=True,
        )

page.ColombiaAutomaticBuilder = FakeBuilder
...
"""
    ...
    scope = next(radio for radio in app.radio if radio.label == "Alcance de la construcción")
    app = scope.set_value("Selección personalizada").run()
    selection = next(
        multiselect
        for multiselect in app.multiselect
        if multiselect.label == "Variables de la canasta CAME"
    )
    ...
    run_button = next(
        ...
        if button.label == "Construir o continuar por etapas"
    )
...
def test_first_base_dates_allow_1990_through_today(monkeypatch) -> None:
    import came.ui.pages.data_maintenance as page
    ...
    last_call = next(call for call in calls if call[0] == "Último día a consultar")
    assert first_call[1]["min_value"] == date(1990, 1, 1)
    assert first_call[1]["max_value"] == date.today()
    assert last_call[1]["min_value"] == date(1990, 1, 1)
    assert last_call[1]["max_value"] == date.today()
...
def test_data_maintenance_delivers_an_advance_zip_after_a_source_error(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("CAME_RUNTIME_STORAGE", str(tmp_path / "runtime"))
    script = """
...
result = BuildResult(
    ...
    data=data,
    status=pd.DataFrame(),
    errors=["Una etapa no respondió"],
)
stored = _create_download_package(
    result,
    country="COL",
    build_id="partial-error",
    additional_sheets={},
    build_notes=[],
    allow_partial=True,
)
_show_result(
    {
        ...
        "warnings": [],
        "errors": result.errors,
        "package_directory": str(stored.directory),
        "country": "COL",
        "run_complete": False,
        "pending_stages": ["Precio de bolsa nacional"],
    },
    "partial_error",
)
"""
    ...
    assert not app.exception
    assert "Descargar ZIP de avance para GitHub" in [
        button.label for button in app.get("download_button")
    ]
