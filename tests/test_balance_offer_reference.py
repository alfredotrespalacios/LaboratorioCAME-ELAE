...
def test_balance_matches_reference_workbook_normal_and_nino() -> None:
    ...
    table = pd.DataFrame(
        {
            "Tecnología": ["Hidráulica", "Gas", "Carbón", "ACPM", "Combustóleo", "Bagazo", "Solar", "GLP", "Jet-A1", "Eólica", "Biogás"],
...
def test_offer_curve_matches_reference_availability_and_marginal_unit() -> None:
    ...
    table = pd.DataFrame(
        {
            "Tecnología": ["Hidráulica", "Gas", "Carbón", "ACPM", "Combustóleo", "Bagazo", "Solar", "GLP", "Jet-A1", "Eólica", "Biogás"],
            "Disponibilidad_GWh_día": [value * fp * 24 / 1000 for value, fp in zip(capacity, factor, strict=True)],
