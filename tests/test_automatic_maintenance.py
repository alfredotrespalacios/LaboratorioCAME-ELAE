...
def test_target_selection_takes_priority_in_a_partial_package() -> None:
    metadata = {
        "selected_options": ["demand"],
        "target_options": ["demand", "spot_price", "availability"],
    }

    assert automatic.selected_options_from_metadata(metadata) == {
        "demand",
        "spot_price",
        "availability",
    }
...
def test_automatic_builder_pauses_after_a_bounded_group_and_can_continue(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("CAME_RUNTIME_STORAGE", str(tmp_path / "runtime"))
    calls: list[set[str]] = []
    checkpoints: list[automatic.AutomaticBuildOutcome] = []
    ...
    monkeypatch.setattr(automatic, "ColombiaMonthlyBuilder", FakeBuilder)
    builder = automatic.ColombiaAutomaticBuilder(build_id="bounded-test")
    first = builder.build(
        "2024-01-01",
        "2024-01-31",
        selected_options={"demand", "spot_price"},
    ...
    assert calls == [{"demand"}]
    assert not first.run_complete
    assert first.pending_stages == ["Precio de bolsa nacional"]
    assert first.processed_this_run == 1
    assert checkpoints[-1].pending_stages == ["Precio de bolsa nacional"]
    ...
    assert calls == [{"demand"}, {"spot_price"}]
    assert second.run_complete
    assert not second.pending_stages
    assert set(second.result.data["series_id"]) == {"col_test_demand", "col_test_spot_price"}


def test_failed_stage_keeps_existing_data_and_emits_a_checkpoint(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("CAME_RUNTIME_STORAGE", str(tmp_path / "runtime"))
    checkpoints: list[automatic.AutomaticBuildOutcome] = []
    ...
    class FailingBuilder:
        ...
        def build(self, *args, **kwargs) -> BuildResult:
            return BuildResult(
                country="COL",
                data=pd.DataFrame(columns=automatic.LONG_COLUMNS),
                status=pd.DataFrame(
                    [{"Fuente": "XM", "Variable": "Demanda", "Estado": "Error"}]
                ),
                errors=["XM no respondió"],
            )
    ...
    monkeypatch.setattr(automatic, "ColombiaMonthlyBuilder", FailingBuilder)
    existing = pd.DataFrame([_row("col_base_existente", "Existente")])
    outcome = automatic.ColombiaAutomaticBuilder(build_id="failure-test").build(
        "2024-01-01",
        "2024-01-31",
        existing=existing,
        selected_options={"demand"},
    ...
    assert outcome.result.errors == ["XM no respondió"]
    assert outcome.failed_stages == ["Demanda nacional"]
    assert outcome.pending_stages == ["Demanda nacional"]
    assert set(outcome.result.data["series_id"]) == {"col_base_existente"}
    assert checkpoints and checkpoints[-1].result.errors == ["XM no respondió"]


def test_historical_period_confirmed_without_data_is_not_queried_again(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv("CAME_RUNTIME_STORAGE", str(tmp_path / "runtime"))
    ...
    class EmptyBuilder:
        ...
        def build(self, start, end, *args, **kwargs) -> BuildResult:
            calls.append((pd.Timestamp(start), pd.Timestamp(end)))
            return BuildResult(
                country="COL",
                data=pd.DataFrame(columns=automatic.LONG_COLUMNS),
                status=pd.DataFrame(
                    [{"Fuente": "XM", "Variable": "Demanda", "Estado": "Sin datos"}]
                ),
            )
    ...
    monkeypatch.setattr(automatic, "ColombiaMonthlyBuilder", EmptyBuilder)
    first = automatic.ColombiaAutomaticBuilder(build_id="no-data-source").build(
        "1990-01-01",
        "1990-12-31",
        selected_options={"demand"},
    )
    assert len(calls) == 1
    assert first.run_ledger[0]["status"] == "no_data"

    second = automatic.ColombiaAutomaticBuilder(build_id="no-data-published").build(
        "1990-01-01",
        "1990-12-31",
        selected_options={"demand"},
        existing_ledger=first.run_ledger,
    )

    assert len(calls) == 1
    assert second.run_complete
    assert second.reused_published == ["Demanda nacional"]
    assert "Sin datos confirmado" in set(second.result.status["Estado"])


def test_current_month_is_always_refreshed(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("CAME_RUNTIME_STORAGE", str(tmp_path / "runtime"))
    calls = 0
    ...
    builder = automatic.ColombiaAutomaticBuilder(build_id="current-month-test")

    first = builder.build(start, now, selected_options={"demand"})
    second = builder.build(
        start,
        now,
        selected_options={"demand"},
    ...
    assert calls == 2
    assert first.run_ledger[0]["status"] == "partial"
    assert second.run_ledger[0]["status"] == "partial"
