...
def test_current_month_can_be_packaged_as_partial_when_explicitly_allowed() -> None:
    data = pd.DataFrame(
        [monthly_row("2024-02-01", 1.0), monthly_row("2024-03-01", 2.0)]
    )
    ...
    assert package.validation.ok
    assert package.metadata["last_complete_month"] == "2024-02-01"
    assert package.metadata["last_available_month"] == "2024-03-01"
    assert package.metadata["partial_month"] == "2024-03-01"
