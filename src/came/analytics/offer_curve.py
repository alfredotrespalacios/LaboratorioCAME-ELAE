...
def _poly_fit(
    ...
    if len(np.unique(x)) <= degree:
        return FitResult(names[degree], [], "", None, None, None, None, "Observaciones insuficientes")
...
def _exponential_fit(
    ...
    if valid.sum() < 2 or len(np.unique(x[valid])) < 2:
        return FitResult("Exponencial", [], "", None, None, None, None, "Requiere precios positivos")
