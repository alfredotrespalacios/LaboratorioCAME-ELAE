...
def summary_indicators(frame: pd.DataFrame, value_column: str = "value") -> dict[str, float | None]:
    ...
    if len(values) > 1:
        result["variación_periodo_pct"] = float((values.iloc[-1] / values.iloc[-2] - 1) * 100) if values.iloc[-2] else None
    if len(values) > 12:
        result["variación_anual_pct"] = float((values.iloc[-1] / values.iloc[-13] - 1) * 100) if values.iloc[-13] else None
    return result
