...
def _theil_u(observed: np.ndarray, predicted: np.ndarray) -> float | None:
    ...
    return (
        float(np.sqrt(np.mean((observed - predicted) ** 2)) / denominator) if denominator else None
    )
...
def regression_metrics(observed: object, predicted: object) -> dict[str, float | None]:
    ...
    nonzero = y != 0
    mape = (
        float(np.mean(np.abs((y[nonzero] - y_hat[nonzero]) / y[nonzero])) * 100)
        if nonzero.any()
        else None
    )
    return {
...
def fit_supervised(
    ...
    *,
    ...
    predictions["residual_estandarizado"] = (
        predictions["residual"] / residual_std
        if residual_std and np.isfinite(residual_std)
        else np.nan
    )
    ...
    if use_standardize:
        warnings.append(
            "Las variables explicativas se estandarizaron usando solo el periodo de entrenamiento."
        )
    warnings.append(
...
def naive_forecast(
    series: pd.Series, horizon: int, seasonal_period: int | None = None
) -> np.ndarray:
