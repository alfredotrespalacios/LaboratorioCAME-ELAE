...
def deduplicate_unserved_demand(
    ...
    *,
    ...
    selected["month"] = (
        selected["datetime"].dt.tz_convert("UTC").dt.tz_localize(None).dt.to_period("M").dt.to_timestamp().dt.tz_localize("UTC")
    )
    monthly = selected.groupby("month", as_index=False)["GWh"].sum().rename(columns={"month": "datetime"})
    ...
    if (audit["selected_hierarchy"] == "desconocido").any():
        warnings.append("Algunas observaciones no tenían jerarquía identificable; revise la auditoría.")
