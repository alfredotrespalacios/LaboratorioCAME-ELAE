...
def _excel_column_width(series: pd.Series, header: object) -> int:
    """Calcula un ancho finito incluso si la columna solo contiene valores vacíos."""

    lengths = series.dropna().astype(str).str.len().head(300)
    content_width = float(lengths.quantile(0.90)) if not lengths.empty else 0.0
    if not np.isfinite(content_width):
        content_width = 0.0
    return int(min(max(len(str(header)) + 2, content_width + 2, 10), 42))
    ...
    with pd.ExcelWriter(buffer, engine="xlsxwriter", datetime_format="yyyy-mm-dd hh:mm") as writer:
        ...
        for sheet_name, frame in sheets.items():
            ...
            for col_idx, column in enumerate(frame.columns):
                ...
                width = _excel_column_width(frame[column], column)
