...
def inspect_quality(
    ...
    *,
    ...
    req_start = (
        pd.to_datetime(requested_start, utc=True) if requested_start is not None else received_start
    )
    ...
    if req_start is not None and req_end is not None:
        ...
        if expected is not None and len(expected):
            observed = pd.DatetimeIndex(
                valid_dt.dt.floor("h") if frequency == "hourly" else valid_dt.dt.normalize()
            )
            if frequency == "monthly":
                observed = (
                    observed.tz_localize(None).to_period("M").to_timestamp().tz_localize("UTC")
                )
    ...
    if missing_expected:
        warnings.append(
            f"Faltan {missing_expected} periodos esperados dentro del rango solicitado."
        )
...
def safe_divide(numerator: object, denominator: object) -> np.ndarray:
    ...
    return np.divide(
        num, den, out=np.full(np.broadcast_shapes(num.shape, den.shape), np.nan), where=den != 0
    )
