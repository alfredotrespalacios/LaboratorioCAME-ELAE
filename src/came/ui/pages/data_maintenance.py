    ...
    *,
    ...
    if operation == OPERATION_UPDATE:
        ...
        if start_ts.date() > complete:
            st.success(
                "La fecha general ya contiene el último mes completo. La ejecución comprobará "
                "además la cobertura de cada variable y reparará cualquier rezago."
            )
            return start_ts.date(), complete_end, existing, None, None
        start = start_ts.date()
...
def _colombia_section(timeout: int) -> None:
    ...
    operation = st.radio(
        ...
        operations,
        index=operations.index(OPERATION_BUILD) if partial_seed else 0,
        key="maintenance_col_operation",
    )
    if partial_seed:
        st.info(
            "Se encontró la Demanda ya construida. La operación inicial se ajustó para completar "
            "automáticamente la primera base; no necesita volver a seleccionar las variables."
        )
    ...
    published_selection = selected_options_from_metadata(metadata)
    ...
    clear = st.checkbox(
        "Borrar los avances automáticos y volver a consultar todos los bloques",
        value=False,
        key="maintenance_col_clear",
    )
    st.caption(
        "La aplicación divide automáticamente la selección en etapas y guarda cada una. "
        "Si una fuente falla, pulse de nuevo el mismo botón sin borrar los avances: solo se "
        "reintentará lo pendiente. Cada bloque tiene tres intentos automáticos."
    )
    button_label = (
        "Construir o reanudar automáticamente"
        if operation != OPERATION_UPDATE
        else "Actualizar y reparar automáticamente"
    )
    if period and st.button(
        ...
        target_first = pd.Timestamp(start).to_period("M")
        target_last = pd.Timestamp(end).to_period("M")
        metadata_first = pd.to_datetime(metadata.get("first_month"), errors="coerce")
        metadata_last = pd.to_datetime(metadata.get("last_complete_month"), errors="coerce")
        published_covers_period = (
            pd.notna(metadata_first)
            and pd.notna(metadata_last)
            and metadata_first.to_period("M") <= target_first
            and metadata_last.to_period("M") >= target_last
        )
        completed_options = (
            published_selection
            if operation == OPERATION_BUILD and published_covers_period
            else set()
        )
        with st.spinner("Procesando automáticamente la canasta por etapas…"):
            outcome = builder.build(
                ...
                existing_option_series=option_series_from_metadata(metadata),
                repair_coverage=(
        ...
        if not result.errors:
            ...
            global_detail.success(
                f"Etapas terminadas: {len(outcome.completed_stages)} procesadas y "
                f"{len(outcome.reused_published)} reutilizadas."
            )
        package = _create_download_package(
            ...
            build_notes=[
                ...
                "Construcción automática por etapas reanudables · Laboratorio CAME 1.5.1.",
