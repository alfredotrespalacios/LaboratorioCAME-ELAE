...
@st.cache_data(show_spinner=False, ttl=3600)
def xm_unserved(start: object, end: object, timeout: int) -> tuple[pd.DataFrame, pd.DataFrame, list[str]]:
    result = unserved_demand(XMProvider(timeout=timeout), start, end)
    return result.monthly, result.hierarchy_audit, result.warnings


@st.cache_data(show_spinner=False, ttl=3600)
def xm_generation_technology(start: object, end: object, frequency: str, timeout: int) -> pd.DataFrame:
    return generation_by_technology(XMProvider(timeout=timeout), start, end, frequency)


@st.cache_data(show_spinner=False, ttl=3600)
def xm_generation_resources(start: object, end: object, timeout: int) -> pd.DataFrame:
