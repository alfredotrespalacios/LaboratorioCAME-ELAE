...
def resource_catalog(provider: XMProvider) -> pd.DataFrame:
    ...
    if resources.empty:
        return pd.DataFrame(
            columns=["resource_code", "resource_name", "technology", "company_code", "energy_source"]
        )
    rename = {
        "Code": "resource_code",
        "code": "resource_code",
    ...
    data = resources.rename(columns={key: value for key, value in rename.items() if key in resources})
    if "resource_code" not in data:
        data["resource_code"] = pd.NA
...
def agent_catalog(provider: XMProvider) -> pd.DataFrame:
    ...
    return agents.rename(columns={"Code": "company_code", "Name": "company_name"})
...
def national_demand(provider: XMProvider, start: object, end: object, frequency: str) -> pd.DataFrame:
...
def capacity_by_technology(
    ...
    selected_utc = selected.tz_localize("UTC") if selected.tzinfo is None else selected.tz_convert("UTC")
    raw = provider.fetch(
        "CapEfecNeta",
        "Recurso",
...
def build_integrated_market(
    ...
    *,
    ...
    try:
        ...
        status_rows.append(
            {"Variable": "Generación por tecnología", "Estado": "No disponible", "Detalle": str(exc)}
        )
    for metric, name, unit in (
        ...
        def loader(
            ...
            rule = "sum" if target in {"GWh", "COP"} else "mean"
            return frame.groupby(pd.Grouper(key="datetime", freq="MS"))["value"].agg(rule).reset_index()
    ...
    if include_macro:
        ...
        try:
            ...
            trm = trm.groupby(pd.Grouper(key="datetime", freq="MS"))["TRM_COP_USD"].mean().reset_index()
            series.append(trm)
            status_rows.append(
                {"Variable": "TRM_COP_USD", "Estado": "Disponible", "Unidad": "COP/USD", "Observaciones": len(trm)}
            )
        except Exception as exc:
        ...
        try:
            ...
            series.append(oni)
            status_rows.append(
                {"Variable": "ENSO_ONI", "Estado": "Disponible", "Unidad": "°C", "Observaciones": len(oni)}
            )
        except Exception as exc:
            warnings.append(f"ENSO: {exc}")

    if not series:
        return IntegratedMarketResult(pd.DataFrame(), pd.DataFrame(status_rows), methodologies, warnings)
    merged = series[0]
