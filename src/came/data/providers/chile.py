...
class ChileProvider(DataProvider):
    ...
    def _read_file(content: bytes, filename: str) -> pd.DataFrame:
        ...
        if suffix in {".xlsx", ".xls"}:
            try:
                ...
                raise SourceContractError(f"No fue posible leer el Excel oficial de Chile: {exc}") from exc
    ...
    def parse_marginal_cost(cls, content: bytes, filename: str) -> pd.DataFrame:
        ...
        result["datetime"] = result["datetime"].dt.tz_localize(
    ...
    @staticmethod
    def national_weighted_price(costs: pd.DataFrame, demand: pd.DataFrame) -> tuple[float, pd.DataFrame]:
    ...
    def fetch_configured_url(self, url: str, filename: str | None = None) -> bytes:
        try:
            ...
            raise SourceUnavailableError(f"No fue posible descargar el archivo oficial de Chile: {exc}") from exc
