...
class XMProvider(DataProvider):
    ...
    def catalog(self, refresh: bool = False) -> pd.DataFrame:
        ...
        try:
            ...
            raise SourceUnavailableError(f"No fue posible consultar el catálogo de XM: {exc}") from exc
        ...
        if catalog.empty or not required.issubset(catalog.columns):
            raise SourceContractError("El catálogo de XM cambió y no contiene las columnas esperadas.")
    ...
    @staticmethod
    def _chunks(start: object, end: object, max_days: int) -> list[tuple[pd.Timestamp, pd.Timestamp]]:
    ...
    def _parse_items(
        ...
        *,
        ...
        for payload in payloads:
            for item in payload.get("Items", []):
                ...
                for entity in entities:
                    ...
                    code = entity.get("Code") or values.get("code") or values.get("Code") or entity_type
                    if frequency == "hourly":
                        value_items = []
        ...
        if data.empty:
            return pd.DataFrame(columns=["datetime", "value", "entity_type", "entity_id", "entity_name"])
        data["datetime"] = pd.to_datetime(data["datetime"], errors="coerce").dt.tz_localize(
            "America/Bogota"
        ).dt.tz_convert("UTC")
    ...
    def healthcheck(self) -> dict[str, Any]:
        ...
        sample = self.fetch("DemaSIN", "Sistema", date(2024, 1, 1), date(2024, 1, 2), target_unit="GWh")
        return {
