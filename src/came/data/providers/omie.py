...
class OmieProvider(DataProvider):
    ...
    def parse_price_file(text: str) -> pd.DataFrame:
        ...
        base_date = pd.to_datetime(
            dict(year=data["year"].astype(int), month=data["month"].astype(int), day=data["day"].astype(int))
        )
    ...
    def fetch_prices(self, start: object, end: object) -> DataResult:
        ...
        if not frames:
            raise SourceUnavailableError("OMIE no devolvió ningún archivo en el periodo solicitado.")
    ...
    def healthcheck(self) -> dict[str, Any]:
        ...
        return {"source": self.source_name, "ok": len(frame) in {24, 25, 92, 96, 100}, "sample_rows": len(frame)}
