...
def selected_options_from_metadata(metadata: dict[str, object]) -> set[str]:
    ...
    selected = metadata.get("selected_options")
...
class ColombiaAutomaticBuilder:
    ...
    def _stage_period(
        ...
        option_next = latest.max().tz_localize(None) + pd.offsets.MonthBegin()
    ...
    def build(
        ...
        *,
        ...
        for index, stage in enumerate(queue, start=1):
            ...
            result = self.stages.load(signature)
            if result is not None:
                ...
                if result.errors:
                    ...
                    for approved in results:
                        partial = merge_monthly_data(partial, approved.data)
                    status_frames = [approved.status for approved in results if not approved.status.empty]
                    if synthetic_status:
                    ...
                    failed = BuildResult(
                        ...
                        warnings=[warning for approved in results for warning in approved.warnings],
                ...
                try:
                    ...
                    result.errors.append(f"No fue posible guardar la etapa {stage.label}: {exc}")
                    return AutomaticBuildOutcome(
                        result,
                        sorted(selected),
                        option_series,
        ...
        data = base.copy()
        for result in results:
            data = merge_monthly_data(data, result.data)
        ...
        final = BuildResult(
            ...
            validation=(
                pd.concat(validations, ignore_index=True).drop_duplicates().reset_index(drop=True)
                if validations
                else pd.DataFrame()
            ),
