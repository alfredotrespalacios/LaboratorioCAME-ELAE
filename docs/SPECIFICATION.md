# Laboratorio CAME — especificación funcional v1.5.2
...
Desde la versión 1.5.1, la canasta completa se procesa mediante una cola automática. Cada etapa
terminada se conserva fuera del repositorio observado, puede recuperarse después de un rerun y se
integra en un solo paquete final. El JSON registra `selected_options` y `option_series`; la
actualización usa esa trazabilidad para conservar la misma canasta y revisar su cobertura sin que el
usuario vuelva a seleccionar variables.

Desde la versión 1.5.2, Colombia admite fechas entre 1990 y el día actual, con intervalos de máximo
cinco años. Una ejecución procesa como máximo tres etapas nuevas o continúa hasta cumplir el
presupuesto temporal después de cerrar una etapa. Cada etapa aprobada produce un paquete
recuperable; un fallo posterior no impide descargar el ZIP de avance. El JSON conserva la operación,
el periodo solicitado, la canasta objetivo, la bitácora y las etapas completas, pendientes y
fallidas. El mes en curso se admite como parcial y permanece elegible para una actualización
posterior. La interfaz explica que el usuario debe reemplazar conjuntamente Parquet, Excel y JSON en
GitHub y que la aplicación nunca publica automáticamente.
