...
La versión 1.5.2 contiene una introducción de uso, los 19 módulos acordados, exportaciones Excel/PDF,
...
puede construir la primera base, agregar meses o variables faltantes, recalcular un periodo,
reanudar bloques aprobados, validar el resultado y descargar un ZIP completo o de avance para
GitHub. Los periodos colombianos se seleccionan entre 1990 y la fecha actual, con un máximo de cinco
años por intervalo. La canasta se divide automáticamente en grupos cortos, conserva cada etapa
aprobada y comprueba la cobertura por variable durante las actualizaciones. No es un módulo de
análisis y no se utiliza durante la operación normal.
...
reducir memoria. Si existe un error de fuente, el último avance aprobado se empaqueta, los
pendientes se escriben en el JSON y la pantalla permite descargar o continuar. La ausencia
explícita de una serie complementaria todavía no publicada queda como advertencia de cobertura.
Ninguna serie se impone como obligatoria: el paquete contiene exclusivamente las variables objetivo
y sus columnas derivadas correspondientes. El mes actual puede incluirse marcado como parcial.
